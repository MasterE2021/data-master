  ...
  async fetchRows(offset, limit) {
    ...
    const res = await fetch('http://127.0.0.1:8000/data/preview', {
      ...
      body: JSON.stringify({
        ...
        page: page,
        page_size: limit,
        total: this.totalRows || 0,   // 若已有 total 则传给后端跳过 count
        columns: []                   // 空数组表示获取所有列
      })
    });
    ...
    // 首次加载时自动创建列定义（基于后端返回的 columns）
    if (!allColumns.value.length) {
      const cols = result.columns.map(colName => ({
        key: colName,
        title: colName,
        width: 120         // 默认列宽，后续可通过拖拽和持久化调整
      }));
      allColumns.value = cols;
      columnOrder.value = cols.map(c => c.key);
      for (const c of cols) {
        columnWidths[c.key] = c.width;
      }
    }

    this.totalRows = result.total;   // 更新总行数
    const list = result.data;
    return {rows: list, hasMore: (offset + limit) < this.totalRows};
  },
...
onMounted(async () => {
  // 1) 首批数据加载，同时自动初始化列定义（由 fetchRows 内部完成）
  await loadMore()

  // 2) 加载并合并用户已持久化的列配置
  const cfg = await mockBackend.loadUserConfig()
  if (cfg) {
    const cols = allColumns.value   // 此时列定义已存在
